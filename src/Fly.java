...
public class Fly
{
    ...
    */
    ...
    public void update(int direction) {
        ...
        // Adjust the value of the row or column according to the direction
        switch (direction) {
            ...
            case FlyWorldGUI.SOUTH:
                newRow++;
                break;
            case FlyWorldGUI.EAST:
                newCol++;
                break;
            case FlyWorldGUI.WEST:
                newCol--;
                break;
        }
    
        // Check that the new location is legitimate
        if (world.isValidLoc(newRow, newCol)) {
            location.removeFly(); // Remove fly from current position
            GridLocation newLocation = world.getLocation(newRow, newCol);
            newLocation.setFly(this); // Place the fly in its new location
            location = newLocation; // Update the fly position reference
        }
    }
}
